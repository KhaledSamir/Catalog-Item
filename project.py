...
def showItems(category_id):
    ...
    return render_template('Items.html' , category = category , items = items , creator = session)
...
def addCatItem(category_id):
    ...
    if request.method == 'POST':
        ...
        title=request.form['title'], category_id=category_id)
        db.add(newItem)
        db.commit()
        flash('Item Created!')
...
@app.route('/category/<int:category_id>/Items/<int:catitem_id>/edit', methods=['GET', 'POST'])
def editCatItem(category_id, catitem_id):
    ...
    if request.method == 'POST':
        ...
        return render_template('editCatItem.html' , item = editedItem, category_id=category_id, item_id=catitem_id)


# Delete a menu item
@app.route('/category/<int:category_id>/Items/<int:catitem_id>/delete', methods=['GET', 'POST'])
def deleteCatItem(category_id, catitem_id):
    if 'username' not in session:
        return redirect('/login')
    restaurant = db.query(Category).filter_by(id=category_id).one()
...
def addCategory():
    if session.get('user_id') is None:
        flash('You are not unauthorized to do this action , please login first to be able to do that!')
...
def logout():
    ...
    if result['status'] == '200':
        ...
        msg= 'Failed to revoke token for given user.' + session.get('access_token')
        return msg
